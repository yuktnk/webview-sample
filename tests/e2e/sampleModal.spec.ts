...
test.describe('パスパラメータ形式 (/sampleModal/$from/$serviceType)', () => {
  test.describe('正常系', () => {
    test('sample_a/type_1 - データが表示される', async ({ page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: 'SampleA Type1 データ' }),
      ).toBeVisible()
      await expect(page.getByText('値: 12345')).toBeVisible()
    })

    test('sample_a/type_2 - リスト形式のデータが表示される', async ({
      page,
    }) => {
      await page.goto('/sampleModal/sample_a/type_2')
      await expect(
        page.getByRole('heading', { name: 'SampleA Type2 データ' }),
      ).toBeVisible()
    ...
    test('sample_b/type_1 - データが表示される', async ({ page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: 'SampleB Type1 データ' }),
      ).toBeVisible()
  ...
  test.describe('バリデーション', () => {
    test('無効なfromパラメータ - Zodバリデーションエラーが表示される', async ({
      page,
    }) => {
