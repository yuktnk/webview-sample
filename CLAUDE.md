...
| 条件クラス名           | clsx                                                      | 2.x        |
| Lint・Format          | Biome（Formatter + 汎用 Linter）                          | 2.x        |
| Lint（補完）          | ESLint（Hooks・Query・a11y 等プラグイン専用）              | 9.x        |
| Gitフック              | lefthook                                                  | 2.x        |
...
| **[docs/adr/](./docs/adr/)**           | ADR（Architecture Decision Record）— なぜそう決めたかの記録 | 開発者・AI  |
...
- [architecture.md](.claude/rules/architecture.md) — 設計原則・ディレクトリ構成
- [file-placement.md](.claude/rules/file-placement.md) — コンポーネント・フック・アセットの配置ルール
- [routing.md](.claude/rules/routing.md) — ルーティング設計・パラメータ定義・エラーハンドリング
- [naming.md](.claude/rules/naming.md) — 命名規則（ESLint 自動チェック範囲・手動で守る規則）
- [api-design.md](.claude/rules/api-design.md) — API 設計ポリシー・1-1-1-1 ルール・エンドポイント一元管理
- [testing.md](.claude/rules/testing.md) — テスト戦略・Storybook・Playwright・Vitest・MSW ルール
- [quality.md](.claude/rules/quality.md) — 品質ピラミッド・静的解析・ライブラリ健全性
- [ci.md](.claude/rules/ci.md) — lefthook・CI/CD（Drone）・ブランチ保護
- [review.md](.claude/rules/review.md) — AI コードレビュー観点
- [native-bridge.md](.claude/rules/native-bridge.md) — NativeBridge 使用方法・iOS / Android 差異
- [design-system.md](.claude/rules/design-system.md) — デザインシステム・トークン管理・単位統一・arbitrary value 禁止
- [docs-generation.md](.claude/rules/docs-generation.md) — ドキュメント自動生成スキルの使い方
